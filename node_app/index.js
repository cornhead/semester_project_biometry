...
const PRNG_SEED = "This is the seed for the PRNG!";

const LEN_RANDOMNESS = 128;
...
function sample_random_field_element(){
    var r = Math.floor(Math.random() * max_r);
    return r;
}

function sample_random_128_bits(){
    var bits = [];
    for (var i = 0; i < 16; i += 1){
        var num = Math.floor(Math.random() * (2**8));
...
function prng_n_field_elements(seed, n){
    var nums = [];
    var prng = seedrandom(seed);

    for (var i = 0; i < n; i += 1){
        nums.push(Math.floor(prng.double() * max_r));
    }
...
async function test(file_path){
    ...
    for (var i = 0; i < len; i += 1){
        ...
        var r_model = sample_random_128_bits();
        var r_probe = sample_random_128_bits();
...
async function prove_internal(input_json){
    ...
    input_json["prng_stream"] = prng_n_field_elements(PRNG_SEED, input_json["probe"].length+LEN_RANDOMNESS);
...
async function verify_internal(proof, public_signals) {
    ...
    var len_prng_stream = public_signals.length-4;

    const t0 = performance.now();
    const res = await snarkjs.groth16.verify(vKey, public_signals, proof);
    var res_prng = true;
    var prng_stream = prng_n_field_elements(PRNG_SEED, len_prng_stream);
    for (var i = 0; i < len_prng_stream; i += 1){
        if (prng_stream[i] != public_signals[i+4]){
            res_prng = false;
            break;
        }
    }
